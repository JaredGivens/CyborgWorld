...
int cube_sdf(vec3 pos) {
    ...
    float is_neg = float(all(greaterThanEqual(vec3(0), local_neg_near)));
    ...
    vec4 norm4 = tsf * vec4(local_norm, 0);
    vec4 near4 = tsf * vec4(local_near, 0);
...
void main() {
    uint save_index = gl_GlobalInvocationID.x / kSDimLen;
    uvec3 cell_pos = gl_GlobalInvocationID;
    cell_pos.x &= kSDimLen - 1;
    uint ind = save_index * uint(pow(kSDimLen, 3))
            + cell_pos.x * kSDimLen * kSDimLen
            + cell_pos.y * kSDimLen
            + cell_pos.z;
